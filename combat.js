...
function buildSheetCollections(sheets) {
  ...
  const pushBase = (rawKey, sheet) => {
    const keys = partyLookupKeys(rawKey);
    for (const key of keys) {
...
function getMegaSheetsForBase(collections, baseSheet, pidLike) {
  ...
  for (const rawKey of [pidLike, baseSheet?.pokemon?.id, baseSheet?.pokemon?.name, baseSheet?.linked_pid]) {
    for (const sheet of (collections?.megaByBaseKey?.get?.(partyLookupKeys(rawKey)[0]) || [])) add(sheet);
    const keys = partyLookupKeys(rawKey);
    for (const key of keys) {
...
export class CombatUI {
  ...
  async _loadSheets(trainerName, forceReload = false) {
    ...
    try {
      ...
      this._sheetCollections.set(trainerName, buildSheetCollections([]));
    }
  }
